          ...
          <div className="mt-2 mb-5 flex items-center justify-between">
            <div className="flex flex-col divide-y w-full">
              <div className="flex flex-col pb-2">
                <p className="font-bold">Tarif Driver + BBM</p>
                {data.include.map((item, index)=>{
                  return(
                    item.driver === "yes" && <p key={index}>{formatCurrency(item.price)} / <span>{item.duration} jam</span></p>
                  )
                })}
              </div>
              <div className="flex flex-col pt-2">
                <p className="font-bold">Tarif Lepas Kunci</p>
                {data.include.map((item, index)=>{
                  return(
                    item.driver === "no" ? <p key={index}>{formatCurrency(item.price)} / <span>{item.duration} jam</span></p> : item.driver === "none" ? <p key={index}>Tidak Tersedia</p> : "" 
                  )
                })}
              </div>
            </div>
